...
function createVenueIcon(category: string, isOpen: boolean): L.DivIcon {
  ...
  return L.divIcon({
    className: '',
    html: `<div style="width:34px;height:40px;position:relative;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.35));opacity:${opacity};"><svg viewBox="0 0 34 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M17 0C7.6 0 0 7.6 0 17c0 11.9 17 23 17 23S34 28.9 34 17C34 7.6 26.4 0 17 0z" fill="${color}"/><circle cx="17" cy="17" r="7" fill="white"/></svg></div>`,
    iconSize: [34, 40],
    iconAnchor: [17, 40],
    popupAnchor: [0, -42],
  });
...
function createUserIcon(): L.DivIcon {
  return L.divIcon({
    className: '',
    html: `<div style="width:20px;height:20px;position:relative;"><div style="width:20px;height:20px;border-radius:50%;background:#3b82f6;border:3px solid white;box-shadow:0 0 0 3px rgba(59,130,246,0.35),0 2px 8px rgba(0,0,0,0.3);"></div></div>`,
    iconSize: [20, 20],
    iconAnchor: [10, 10],
  });
}
  ...
  useEffect(() => {
    ...
    venues.forEach((venue) => {
      const marker = L.marker([venue.lat, venue.lng], {
        ...
        .addTo(mapRef.current!)
        .bindPopup(
          `<div style="min-width:160px"><strong style="font-size:14px">${venue.name}</strong><div style="font-size:12px;color:#78716c;margin-top:2px">${venue.address}</div><div style="display:flex;align-items:center;gap:6px;margin-top:6px"><span style="font-weight:600">⭐ ${venue.rating.toFixed(1)}</span><span style="color:${venue.isOpen ? '#10b981' : '#ef4444'};font-weight:600">${venue.isOpen ? 'Open' : 'Closed'}</span></div></div>`
        );
